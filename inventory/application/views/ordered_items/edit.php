...
<?php echo form_open('ordered_items/edit/'.$ordered_items['id'],array("class"=>"form-horizontal")); ?>
<div class="row">
       <div class="col-md-3">
        <label class="control-label">Part Number</label>
        <input type="text" name="part_no" value="<?php echo ($this->input->post('part_no') ? $this->input->post('part_no') : $ordered_items['part_no']); ?>" class="form-control" id="part_no" placeholder="Enter Part number" />
        <span class="text-danger"><?php echo form_error('part_no');?></span>
         </div>

          <div class="col-md-3">
        <label class="control-label">Item Description</label>
        <input type="text" name="description" value="<?php echo ($this->input->post('description') ? $this->input->post('description') : $ordered_items['description']); ?>" class="form-control" id="part_no" placeholder="Enter Item Description" />
...
<!-- 
          <div class="col-md-3"><label class="control-label">Item Description</label><div class="form-group"><select onmouseenter="abc()" class="js-example-basic-single form-control"  name="description" >
              ...
              <?php foreach ($items as $u) { $selected = ($u['id'] == $ordered_items['description']) ? ' selected="selected"' : "";?>
        ...
        <input type="number" name="quantity" value="<?php echo ($this->input->post('quantity') ? $this->input->post('quantity') : $ordered_items['quantity']); ?>" class="form-control" id="quantity" placeholder="Enter Quantity" />
        <span class="text-danger"><?php echo form_error('quantity');?></span>
         </div>
             <div class="col-md-3">
        <label class="control-label">Price per Unit</label>
        <input type="number" name="ppu" value="<?php echo ($this->input->post('ppu') ? $this->input->post('ppu') : $ordered_items['ppu']); ?>" class="form-control" id="ppu" placeholder="Enter Price Per Unit" />
        <span class="text-danger"><?php echo form_error('ppu');?></span>
         </div>
           <div class="col-md-3">
        <label class="control-label">Location, if arrived</label>
        <input type="text" name="location" value="<?php echo ($this->input->post('location') ? $this->input->post('location') : $ordered_items['location']); ?>" class="form-control" id="location" placeholder="Enter Location" />
